...
export const fetchPageAssets = (assets, hasFallback) => (dispatch, getState, sdk) => {
  ...
  return Promise.all(sdkAssets)
    ...
    .catch(e => {
      // If there's a fallback UI and it's a 404, we silently ignore it (likely a missing localized page)
      // Don't log the error or dispatch to Redux to avoid error state
      const is404WithFallback = hasFallback && e.status === 404;
      
      if (is404WithFallback) {
        // Silently return - the base page is already loaded
        return Promise.resolve(null);
      }
      
      // For real errors, log and dispatch to Redux
      log.error(e, 'page-asset-fetch-failed', { assets, version });
      dispatch(pageAssetsError(storableError(e)));
      
      // Re-throw so the caller knows about the error
      throw e;
    });
};
